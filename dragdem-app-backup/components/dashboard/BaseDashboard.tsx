    ...
    <main className="min-h-screen pb-20 bg-deep text-white">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
          <div>
            <h1 className="text-4xl font-montserrat font-bold text-white">Welcome back, <span className="text-primary italic">{userName}!</span></h1>
            <p className="text-gray-400 mt-1 font-inter">Here&apos;s what&apos;s happening with your empire today.</p>
          </div>
          <Link href="/settings/rates" className="flex items-center gap-2 bg-surface border border-gray-800 hover:border-primary transition-colors px-5 py-3 rounded-xl font-bold text-sm">
...
function StatCard({ title, value, trend, icon, trendUp }: any) {
  return (
    <div className="bg-surface border border-gray-800 p-6 rounded-2xl shadow-xl hover:border-primary/30 transition-colors group">
      <div className="flex justify-between items-start mb-4">
        <div className="p-3 bg-deep rounded-xl border border-gray-800 group-hover:border-primary/20 transition-colors">
          {icon}
        </div>
        <div className={`flex items-center gap-1 text-sm font-bold ${trendUp ? 'text-accent' : 'text-red-500'}`}>
          {trend}
          <ArrowUpRight size={14} />
        </div>
      </div>
      <p className="text-gray-400 text-xs uppercase tracking-widest font-bold mb-1 font-montserrat">{title}</p>
      <p className="text-3xl font-bold font-montserrat">{value}</p>
    </div>
  );
}
