      ...
      <div className="container mx-auto px-4 pt-8">
        ...
        {article.category === "Spotlight" ? (
          <div className="space-y-16">
            <SpotlightHero 
              artistName={article.title.split(': ')[1] || article.title}
              tagline={article.excerpt}
              imageUrl={article.imageUrl}
            />
            
            <NarrativeFlow 
              quote={article.content.split('> "')[1]?.split('"')[0] || ""}
              story={article.content.split('###')[1] || article.content}
              stats={[
                { label: "Region", value: article.region || "Global" },
            ...
            {article.performerVanity && (
              <VaultHighlight 
                title={`${article.title.split(': ')[1] || article.title}'s Media Kit`}
                description={`Access the exclusive performance assets and technical riders for ${article.title.split(': ')[1] || article.title}.`}
                vaultItemId={article.id}
              />
            )}
