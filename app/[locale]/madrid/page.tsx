...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "Madrid" });
  
  return {
    title: `${t("title")} | The Professional Backbone for Drag`,
    description: t("description"),
    openGraph: {
      images: ['/assets/madrid_banner.png']
    }
  };
}

export default async function MadridLandingPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations("Madrid");
  ...
  return (
    <main className="min-h-screen bg-black text-white selection:bg-primary selection:text-white">
      ...
      <section className="relative pt-48 pb-32 overflow-hidden">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_0%,rgba(0,255,255,0.1)_0%,transparent_50%)]" />
        
        <div className="container mx-auto px-4 relative z-10 text-center">
          <Badge variant="outline" className="mb-8 px-6 py-2 border-secondary/50 text-secondary uppercase tracking-[0.5em] text-xs font-bold">
            {t("title")}
          </Badge>
          ...
          <div className="flex flex-wrap justify-center gap-6">
            <Button size="lg" className="bg-secondary text-black hover:shadow-[0_0_30px_rgba(0,255,255,0.4)] transition-all px-10 py-8 h-auto text-sm font-bold uppercase tracking-widest">
              {t("cta")}
            </Button>
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-col lg:flex-row-reverse items-center gap-24">
            <div className="flex-1 relative group">
              <div className="absolute -inset-4 bg-secondary/20 blur-3xl opacity-50 group-hover:opacity-100 transition-opacity" />
              <div className="relative rounded-[3rem] overflow-hidden border border-white/10 aspect-[4/5] md:aspect-square lg:aspect-[4/5]">
                <Image 
                  src="/assets/madrid_banner.png" 
                  alt="Madrid Drag Scene" 
                  fill 
                  className="object-cover"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent" />
                <div className="absolute bottom-12 right-12">
                  <Image src="/assets/madrid_badge.png" alt="Madrid Pro Badge" width={120} height={120} className="drop-shadow-2xl" />
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 space-y-12">
              ...
              <div className="grid gap-8">
                <div className="flex gap-6 p-8 rounded-3xl bg-black/40 border border-white/5 hover:border-secondary/30 transition-colors">
                  <div className="shrink-0 w-12 h-12 rounded-xl bg-secondary/10 flex items-center justify-center">
                    <CheckCircle2 className="text-secondary" />
                  </div>
                  <div>
                    <h3 className="text-xl font-bold mb-2">{tPro("london_vat")}</h3>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto space-y-12">
             <div className="relative inline-block">
                <div className="absolute inset-0 bg-secondary/30 blur-[100px] rounded-full" />
                <Image 
                  src="/assets/madrid_seal.png" 
                  alt="Madrid Labor Dignity Seal" 
                  width={250} 
                  height={250} 
                  className="relative drop-shadow-[0_0_50px_rgba(0,255,255,0.3)]"
                />
             </div>
             ...
             <p className="text-2xl text-white/40 font-light italic">
               El estandarte de la excelencia profesional para el mercado español.
             </p>
          </div>
        </div>
      </section>
      ...
      <footer className="py-20 border-t border-white/5 text-center">
        <div className="container mx-auto px-4">
          <div className="font-montserrat font-black text-2xl tracking-[0.2em] mb-8">DRAGDEM MADRID</div>
